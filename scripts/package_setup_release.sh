...
#
# Writes: dist/dbfb-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix dbfb \
  --exe-name DBFinalBout_Recompiled \
  --display-name "DBFinalBout Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned DBFinalBout disc" \
